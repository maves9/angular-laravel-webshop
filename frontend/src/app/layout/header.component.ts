import { Component, OnInit, computed, Signal, signal, DestroyRef, WritableSignal } from '@angular/core'
...
export class HeaderComponent implements OnInit {
  ...
  readonly totalCount = computed(() => this.cart().reduce((acc, it) => acc + (Number(it?.quantity ?? it?.qty ?? 0)), 0))
  readonly totalPriceDisplay = computed(() => {
    const total = this.cart().reduce((acc, it) => acc + ((Number(it?.price ?? 0)) * (Number(it?.quantity ?? it?.qty ?? 0))), 0)
    return this.formatPrice(total)
  })

  dropdownOpen = false

  constructor(private cartService: CartService, private destroyRef: DestroyRef) {}

  ngOnInit(): void {
    // subscribe and update the raw signal; register cleanup with DestroyRef
