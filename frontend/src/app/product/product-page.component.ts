...
export class ProductPage {
  ...
  public getDisplayDescription(locale = 'en'): string | null {
    ...
    const variantOptions = this.product ? (this.product['variant_options'] as Record<string, Array<any>> | undefined) : undefined;
    if (this.product && this.selected) {
      const parts: string[] = [];
      for (const [variantName, value] of Object.entries(this.selected)) {
        ...
        if (Array.isArray(arr)) {
          ...
          if (entry && entry.descriptions) {
            ...
            if (typeof d === 'string' && d.length > 0) parts.push(d);
          }
        } else {
          ...
          if (combos) {
            for (const c of combos) {
              ...
              if (opts[variantName] === value && (c as any).description) {
                parts.push((c as any).description);
                break;
              }
            }
          }
  ...
  public getVariantTypes(): string[] {
    ...
    const variantOptions = this.product ? (this.product['variant_options'] as Record<string, Array<any>> | undefined) : undefined
    if (variantOptions) {
      return Object.keys(variantOptions);
    }
    const combos = (this.product?.combinations as ProductVariantCombinationWithOptions[] | undefined) || undefined
