...
export class ProductPage {
    ...
    if (combination && 'id' in combination) {
      ...
      try {
        ...
        if (e && typeof e === 'object' && 'message' in e) {
          ...
          else if (obj['message'] && typeof obj['message'] === 'object') msg = JSON.stringify(obj['message'])
        }
  ...
  const entry = arr.find((a) => isVariantEntry(a) && (a.value === value || a === value)) as VariantEntry | undefined
  ...
  const descs = entry.descriptions as Record<string, string>
      ...
      if (!combos) return []
      const found = combos.find((c) => ((c.options as Record<string, string> | undefined) || {})[variantName] === value && (c as unknown as { description?: unknown }).description)
  ...
  public getFinalCombinationDescription(): string | null {
    const combination =
      this.finalCombination && !('error' in this.finalCombination)
        ? (this.finalCombination as ProductVariantCombinationWithOptions)
        : null
      if (!combination || !('description' in (combination as unknown as Record<string, unknown>))) return null
  ...
  const variantOptions = this.product['variant_options'] as Record<string, Array<unknown>> | undefined
    const arr = variantOptions ? (variantOptions[variantName] as unknown[] | undefined) : undefined
    if (Array.isArray(arr))
      return arr.map((a) => {
    ...
    const combos = (this.product?.combinations as ProductVariantCombinationWithOptions[] | undefined) ||
      undefined
    if (!combos || combos.length === 0) return []
